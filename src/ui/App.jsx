  ...
  async function refresh() {
    try {
      ...
      const items = (r.items || []).filter(x =>
        !f || [x.provider, x.credential_type, x.notes || ''].some(s => (s || '').toLowerCase().includes(f))
      )
      setRows(items)
    } catch (e) {
    ...
    <>
      <header>
        ...
        <div className="toolbar">
          <input
            placeholder="API URL (e.g., https://your-server.onrender.com)"
        ...
        <div className="modal-backdrop" style={{ display: 'flex' }}>
          <div className="modal">
            ...
            <footer style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
              <button onClick={async () => {
                if (!form.provider || !form.credential_type) {
                  alert('Provider and Type are required.')
                  return
                }
